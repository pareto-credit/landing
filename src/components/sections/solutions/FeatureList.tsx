...
const FeatureListItem = ({ feature, isActive, onHover }: FeatureListItemProps) => (
  <li
    ...
    className={cn(
      'flex cursor-pointer items-center gap-3 rounded-xl p-3.5 transition-all duration-300 xl:gap-4 xl:p-4',
      isActive
  ...
  >
    ...
    <span className={cn('text-base font-semibold xl:text-lg', isActive ? 'text-[var(--color-text-inverse)]' : 'text-[var(--color-text-muted)]')}>
      {feature.name}
    </span>
  </li>
)
